...
const ReceiptView = ({ data, imageSrc }: ReceiptViewProps) => {
  ...
  // Receipt content (shared between layouts)
  const receiptContent = (
    <div className="bg-white p-5 shadow-lg rounded-sm border-t-[6px] border-gray-800 relative overflow-hidden">
        ...
        {data.items.map((item, idx) => {
          ...
          return (
            <div key={idx} className="pb-3 border-b border-gray-100 last:border-0">
              {/* Row 1: Number + Name + Total */}
              <div className="flex items-start gap-2">
                <button
                  onClick={() => handleTranslate(item.originalName)}
                  className="shrink-0 w-6 h-6 rounded-full bg-orange-500 text-white text-xs font-bold flex items-center justify-center mt-0.5 hover:bg-orange-600"
                >
                  {idx + 1}
                </button>
                ...
                <span className="font-bold text-gray-900 shrink-0 text-base">{formatPrice(item.price, data.currency)}</span>
              </div>
              {/* Row 2: Unit price × Quantity */}
              <div className="ml-8 mt-1 flex items-center gap-2 text-xs">
      ...
      {/* Totals */}
      <div className="border-t-2 border-dashed border-gray-300 mt-4 pt-3 space-y-1.5 text-sm">
      ...
      <div className="mt-6 space-y-3">
        ...
        >
          {saved ? <><CheckCircle size={16} /> 已加入記帳簿</> : <><Bookmark size={16} /> 加入記帳簿</>}
        </button>
      </div>
    </div>
  );
      ...
      {imageSrc && (
        <div className="flex justify-end mb-2 gap-1">
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${layout === 'stack' ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'}`}
            title="上下對照"
          >
            <Rows3 size={16} />
          </button>
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${layout === 'side' ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'}`}
            title="左右對照"
          >
            <Columns2 size={16} />
          </button>
      ...
      {layout === 'side' && imageSrc ? (
        ...
        <div className="flex gap-3">
          <div className="w-1/2 shrink-0">
            <div className="sticky top-[53px] rounded-xl overflow-hidden border border-gray-200 shadow-sm">
              <img src={imageSrc} alt="Receipt" className="w-full" />
            </div>
          </div>
          <div className="w-1/2">
            {receiptContent}
        ...
        /* Stack: receipt only (photo is in sticky header) */
        receiptContent
      )}
    </div>
  );
